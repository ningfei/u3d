...
void CreateMaterial(SceneConverter &converter, vtkActor *anActor, bool emissive, Material &materialResource) {
    ...
    MaterialResourceList *pMaterialResources = static_cast< MaterialResourceList * >( Resources.GetResourceList(
            IDTF_MATERIAL));
...
//  vtkActor2D *anTextActor2D, *aPart2D;
...

    {
        ...
        wchar_t *wU3DFileName = new wchar_t[wcslen(this->FileName) + 1 + 4];
        ...
        if (IFXSUCCESS(result)) {
            {
                ...
                {
                    LightResourceList *pLightResources = static_cast< LightResourceList * >( Resources.GetResourceList(
                            IDTF_LIGHT));
                    ...
                    // ambient light
                    {
                        ...
                        ambientLightResource.m_color.SetColor(
                                IFXVector4(ren->GetAmbient()[0], ren->GetAmbient()[1], ren->GetAmbient()[2]));
...
//          0 0 sign(c)
                            ...
                            lightResource.m_color.SetColor(
                                    IFXVector4(aLight->GetDiffuseColor()[0], aLight->GetDiffuseColor()[1],
                                               aLight->GetDiffuseColor()[2]));
                            lightResource.m_attenuation.SetPoint(
                                    IFXVector3(aLight->GetAttenuationValues()[0], aLight->GetAttenuationValues()[0],
                                               aLight->GetAttenuationValues()[0]));
                ...
                // do the actors now
                ModelResourceList *pModelResources = static_cast< ModelResourceList * >   ( Resources.GetResourceList(
                        IDTF_MODEL));
                MaterialResourceList *pMaterialResources = static_cast< MaterialResourceList * >( Resources.GetResourceList(
                        IDTF_MATERIAL));
                ShaderResourceList *pShaderResources = static_cast< ShaderResourceList * >  ( Resources.GetResourceList(
                        IDTF_SHADER));
                TextureResourceList *pTextureResources = static_cast< TextureResourceList * > ( Resources.GetResourceList(
                        IDTF_TEXTURE));
...

                        {
                            ...
                            if (representation == VTK_POINTS) {
                                ...
                                for (vtkIdType pid = 0; pid < numPoints; pid++) {
                                    ...
                                    pointSetResource.m_positions.CreateNewElement().SetPoint(
                                            IFXVector3(point[0], point[1], point[2]));
                                }
                                ...
                                if ((numPolys > 0 || numStrips > 0) && representation == VTK_SURFACE) {
                                    ...
                                    if (myActor->GetMapper()->IsA("vtkPolyDataMapper"))
                                        ...
                                        myPolyDataMapper->ShallowCopy(
                                                static_cast<vtkDataSetMapper *>(myActor->GetMapper())->GetPolyDataMapper());
                                    else
                                        continue;
                                    ...
                                    if (colorCoordinates) {
                                        ...
                                        VTK_DECLARE(vtkIdList,
                                                    ucolorCoordinatesIds); // ids of original colors in ucolors
...

                                        {
                                            ...
                                            colorCoordinatesLocator->InitPointInsertion(ucolorCoordinates,
                                                                                        colorCoordinatesBounds,
                                                                                        numColorCoordinates);
                                            ...
                                            for (vtkIdType id = 0; id < numColorCoordinates; id++) {
                                                ...
                                                colorCoordinatesLocator->InsertUniquePoint(colorPoint,
                                                                                           colorCoordinateId);
                                        ...
                                        meshResource.m_modelDescription.basePositionCount = (MeshCompression ? 0
                                                                                                             : numPoints);
                                        ...
                                        for (polys->InitTraversal(); polys->GetNextCell(npts, indx);) {
                                            meshResource.m_facePositions.CreateNewElement().SetData(
                                                    upointIds->GetId(indx[0]), upointIds->GetId(indx[1]),
                                                    upointIds->GetId(indx[2]));
                                        }

                                        if (normals) {
                                            if (cell_normals) {
                                                for (vtkIdType pid = 0; pid < numPolys; pid++) {
                                                    meshResource.m_faceNormals.CreateNewElement().SetData(
                                                            unormalIds->GetId(pid), unormalIds->GetId(pid),
                                                            unormalIds->GetId(pid));
                                                }
                                            } else {
                                                for (polys->InitTraversal(); polys->GetNextCell(npts, indx);) {
                                                    meshResource.m_faceNormals.CreateNewElement().SetData(
                                                            unormalIds->GetId(indx[0]), unormalIds->GetId(indx[1]),
                                                            unormalIds->GetId(indx[2]));
                                                }
                                            }
                                        }
                                        ...
                                        for (polys->InitTraversal(); polys->GetNextCell(npts, indx);) {
                                            meshResource.m_faceTextureCoords.CreateNewElement().m_texCoords.CreateNewElement().SetData(
                                                    ucolorCoordinatesIds->GetId(indx[0]),
                                                    ucolorCoordinatesIds->GetId(indx[1]),
                                                    ucolorCoordinatesIds->GetId(indx[2]));
                                        }

                                        for (vtkIdType pid = 0; pid < numPoints; pid++) {
                                            const double *point = upoints->GetPoint(pid);
                                            meshResource.m_positions.CreateNewElement().SetPoint(
                                                    IFXVector3(point[0], point[1], point[2]));
                                        }

                                        for (vtkIdType pid = 0; pid < numNormals; pid++) {
                                            const double *normal = unormals->GetPoint(pid);
                                            meshResource.m_normals.CreateNewElement().SetPoint(
                                                    IFXVector3(normal[0], normal[1], normal[2]));
                                        }

                                        for (vtkIdType pid = 0; pid < numColorCoordinates; pid++) {
                                            meshResource.m_textureCoords.CreateNewElement().Set(
                                                    (ucolorCoordinates->GetPoint(pid))[0], 0, 0, 0);
                                        }
                                        ...
                                        swprintf(textureName, 255, L"Texture%u", pTextureResources->GetResourceCount());
                                        {
                                            ...
                                            for (int i = 0; i < colorTextureMapSize; i++) {
                                                const unsigned char *color = static_cast<unsigned char *>(colorTextureMap->GetScalarPointer(
                                                        i, 0, 0));
                                        ...
                                        if (colors) {
                                            ...
                                            for (vtkIdType pid = 0; pid < numColors; pid++) {
                                                ...
                                                for (vtkIdType cid = 0; cid < uucolors->GetNumberOfTuples(); cid++) {
                                                    ...
                                                    if (color[0] == uucolor[0] && color[1] == uucolor[1] &&
                                                        color[2] == uucolor[2] && color[3] == uucolor[3]) {
                                                        curColor = cid;
                                                        break;
                                                    }
                                                }
                                            ...
                                            for (vtkIdType cid = 0; cid < uucolors->GetNumberOfTuples(); cid++) {
                                                unsigned char uucolor[4];
                                                uucolors->GetTupleValue(cid, uucolor);
                                                const double ucolor[4] = {uucolor[0] / 255.0, uucolor[1] / 255.0,
                                                                          uucolor[2] / 255.0, uucolor[3] / 255.0};
                                        ...
                                        if (colors) {
                                            ...
                                            for (vtkIdType pid = 0; pid < numColors; pid++) {
                                                ...
                                                shadingDescription.m_textureLayerCount = 0;
                                                meshResource.m_shadingDescriptions.AddShadingDescription(
                                                        shadingDescription);
                                            }
                                        } else {
                                            meshResource.m_modelDescription.shadingCount = 1;
                                        ...
                                        for (vtkIdType pid = 0; pid < numPolys; pid++) {
                                            meshResource.m_faceShaders.CreateNewElement() = (colors ? ucolorIds->GetId(
                                                    pid) : 0);
                                        }
                                        ...
                                        if (colors) {
                                            for (vtkIdType pid = 0; pid < numColors; pid++) {
                                                ...
                                                double intens;
                                                wchar_t materialName[256];
                                                swprintf(materialName, 255, L"Material%u",
                                                         pMaterialResources->GetResourceCount());

                                                Material materialResource;
                                                materialResource.SetName(materialName);
                                                intens = prop->GetAmbient();
                                                materialResource.m_ambient.SetColor(
                                                        IFXVector4(intens * color[0], intens * color[1],
                                                                   intens * color[2]));
                                                intens = prop->GetDiffuse();
                                                materialResource.m_diffuse.SetColor(
                                                        IFXVector4(intens * color[0], intens * color[1],
                                                                   intens * color[2]));
                                                intens = prop->GetSpecular();
                                                materialResource.m_specular.SetColor(
                                                        IFXVector4(intens * color[0], intens * color[1],
                                                                   intens * color[2]));
                                                intens = prop->GetAmbient();
                                                materialResource.m_emissive.SetColor(
                                                        IFXVector4(intens * color[0], intens * color[1],
                                                                   intens * color[2]));
                                                materialResource.m_reflectivity = prop->GetSpecularPower() / 128.0;
                                                // Material transparency
                                                ...
                                                wchar_t shaderName[256];
                                                swprintf(shaderName, 255, L"Shader%u",
                                                         pShaderResources->GetResourceCount());
                                                Shader shaderResource;
                                                shaderResource.SetName(shaderName);
                                        ...
                                        // Use vertex colors, lighting and transparency are not supported by Acrobat <= 9.1
                                    {
                                        ...
                                        if (colors) {
                                            ...
                                            {
                                                for (vtkIdType pid = 0; pid < numPolys; pid++) {
                                                    meshResource.m_faceDiffuseColors.CreateNewElement().SetData(
                                                            ucolorIds->GetId(pid), ucolorIds->GetId(pid),
                                                            ucolorIds->GetId(pid));
                                                }
                                            } else {
                                                for (polys->InitTraversal(); polys->GetNextCell(npts, indx);) {
                                                    meshResource.m_faceDiffuseColors.CreateNewElement().SetData(
                                                            ucolorIds->GetId(indx[0]), ucolorIds->GetId(indx[1]),
                                                            ucolorIds->GetId(indx[2]));
                                                }
                                            }
                                        }
                                        ...
                                        for (vtkIdType pid = 0; pid < numColors; pid++) {
                                            ...
                                            meshResource.m_diffuseColors.CreateNewElement().SetColor(
                                                    IFXVector4(color[0], color[1], color[2]));
                                        }
...
#undef mAddLine
                                    ...
                                    VTK_CREATE(vtkIdList,
                                               lucolorIds);     // indexes in the above list of line colors - 1 per line
                                    ...
                                    VTK_CREATE(vtkIdList,
                                               lupointIdIds); // indexes in the above list of line points - 2 per line
                                    ...
                                    for (vtkIdType pid = 0; pid < lnumLines; pid++) {
                                        lineSetResource.m_linePositions.CreateNewElement().SetData(
                                                lupointIdIds->GetId(pid * 2 + 0), lupointIdIds->GetId(pid * 2 + 1));
                                    }

                                    for (vtkIdType pid = 0; pid < lnumLines; pid++) {
                                        lineSetResource.m_lineShaders.CreateNewElement() = (colors ? lucolorIds->GetId(
                                                pid) : 0);
                                    }

                                    for (vtkIdType pid = 0; pid < lnumPoints; pid++) {
                                        const double *point = upoints->GetPoint(lupointIds->GetId(pid));
                                        lineSetResource.m_positions.CreateNewElement().SetPoint(
                                                IFXVector3(point[0], point[1], point[2]));
                                    }
